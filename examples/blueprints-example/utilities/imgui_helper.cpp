...
struct ImGuiLayoutItem
{
    ...
    ImGuiLayoutItem(ImGuiLayoutItemType type)
    {
        ...
        VertexIndexBegin = VertexIndexEnd = static_cast<ImDrawIdx>(0);
    }
};
...
static void PushLayout(ImGuiLayout* layout)
{
    ...
    window->DC.CurrentLayoutItem = nullptr;
}
...
static void PopLayout(ImGuiLayout* layout)
{
    ...
    {
        window->DC.CurrentLayout = nullptr;
        window->DC.CurrentLayoutItem = nullptr;
    }
}
...
{
    for (ImGuiLayout* child = layout.FirstChild; child != nullptr; child = child->NextSibling)
    {
...
static ImGuiLayout* FindLayout(ImGuiID id, ImGuiLayoutType type)
{
    ...
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    ImGuiLayout* layout = static_cast<ImGuiLayout *>(window->DC.Layouts.GetVoidPtr(id));
    if (!layout)
        return nullptr;
